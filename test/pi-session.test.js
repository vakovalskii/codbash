...
test('active Pi command parsing maps quoted resume path back to session id', () => {
  ...
  const cmd = "omp --resume '/tmp/project with spaces/session'\\''one.jsonl'";
  const parsed = extractPiResumeTargetFromCommand(cmd);
  assert.equal(parsed, resumeTarget);

  const session = findPiSessionByResumeTarget(parsed, [
